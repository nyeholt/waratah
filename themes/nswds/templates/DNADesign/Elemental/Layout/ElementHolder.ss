...
<% else_if $CurrentPage.IsLandingPage || $Parent.OwnerPage.IsLandingPage %>
    ...
    <section class="<% if $Background %>nsw-section--{$Background}<% else %>nsw-block<% end_if %><% if $StyleVariant %> $StyleVariant<% end_if %><% if $ExtraClass %> $ExtraClass<% end_if %>" id="{$Anchor}" data-type="{$ElementShortName}">
